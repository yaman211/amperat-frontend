<template>
  <q-card class="q-pa-md rounded-lg">
    <div class="text-h6 q-mb-md text-weight-medium">
      <q-icon name="request_page" class="q-mr-sm text-secondary" size="32px" />
      عدد الفواتير التي تم ادخالها
    </div>
    <q-separator class="q-mb-md" />
    <Loader v-if="statisticsStore.loading" style="height: 300px" />
    <template v-else-if="!statisticsStore.invoicesCount.labels.length">
      <NoData />
    </template>
    <template v-else>
      <div style="height: 300px; width: 100%">
        <Line :data="statisticsStore.invoicesCount" />
      </div>
    </template>
  </q-card>
</template>
